...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderMicroSteps() {
    microStepsList.innerHTML = appState.tasks.map((step, idx) => `
      <li class="step-item ${step.completed ? 'completed' : ''} ${appState.activeStepIndex === idx ? 'active' : ''}" data-idx="${idx}">
        ...
        ${!step.completed ? `<button class="btn btn-xs btn-secondary btn-step-action" data-idx="${idx}">Hacer</button>` : `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-emerald)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle;"><polyline points="20 6 9 17 4 12/></svg>`}
      </li>
    `).join('');
  ...
  // Lógica global para los botones de mostrar/ocultar contraseña (ojo)
  document.querySelectorAll(".btn-toggle-password").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (targetInput) {
        ...
        // Cambiar el icono SVG del ojo (ojo abierto / ojo tachado)
        btn.innerHTML = isPassword 
          ? `<svg class="eye-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--color-teal)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>`
